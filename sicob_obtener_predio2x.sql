...
CREATE OR REPLACE FUNCTION public.sicob_obtener_predio2x(_opt json)
 RETURNS json
 LANGUAGE plpgsql
AS $function$
DECLARE 
...
--> tolerance (opcional): Distancia maxima (en metros) para el autoajuste automatico de los bordes de "a" hacia los bordes de "b" (snapping). Si no se especifica, no se realiza autoajuste.
--> subfix (opcional): Texto que se adicionara al final del nombre de las capas resultantes.

--VARIABLES DE SALIDA:
--> lyr_over: Nombre de la capa resultante, este es igual al nombre de la capa de entrada mas mas el texto de subfix (si se proporciona) mas el texto “_ppred” . Esta capa siempre sera creada en el esquema “temp” de la BD. 
--> features_inters_cnt: Cantidad de poligonos que SI se han encontrado sus predios.
--> features_diff_cnt: Cantidad de poligonos de los cuales NO se ha encontrado predio.
--> lyr_geojson : Si se ha indicado el valor geojson de entrada como "true", devuelve la capa resultado en formato geojson.
--> lyr_pred: Nombre de la capa que contiene los predios encontrados. Este valor es igual al nombre de la capa de entrada mas el texto de subfix (si se proporciona) mas el texto “_pred” . Esta capa siempre sera creada en el esquema “temp” de la BD.
        ...
        IF COALESCE( (_out->>'features_inters_cnt')::int,0) > 0 THEN  --> Si se han localizado predios.
            ...
            sql := format('
                    ...
                    CASE (lyr_predio->>'subfix')::text 
                        ...
                        WHEN '_tioc' THEN 'CAST(''Territorio Indigena Originario Campesino'' as text)' 
                        ELSE 'CAST(NULL AS text)' 
            ...
            --> Actualizando la superficie del predio en caso de indicar una cobertura de predio/parcela
            IF (lyr_predio->>'subfix')::text = '_parc' THEN
                EXECUTE format('
                    UPDATE
                        %s a
                    SET
                ...
                ', (tbl_name || _subfix || '_ppred') ,(lyr_predio->'lyr_parc'->>'source')::text );
            END IF;
...
--> Complementando informacion de ubicacion politico-administrativo.
    sql := 'SELECT ' || 
    sicob_no_geo_column(
...
--> AGREGANDO INFORMACION DE TITULACION A LOS POLIGONOS UBICADOS EN PARCELAS DE REFERENCIA
	IF COALESCE( (_opt->>'lyr_parc')::text,'') <> '' THEN --> Si se indica cobertura de referencia.
        ...
        tmp := sicob_overlap2x(('{"a":"' || a || '","condition_a":"' || _condition || '","b":"coberturas.parcelas_tituladas","subfix":"_tit","tolerance":"0","add_diff":false, "temp": true}')::json);
...
END IF;
	...
	--Eliminando poligonos con superfice menor a la minima.
...
IF inters_cnt > 0 THEN --> Si se han encontrado poligonos.                  
	...
	IF COALESCE( (_opt->>'lyr_parc')::text,'') <> '' THEN --> Si se indica cobertura de referencia.
        ...
        tmp := sicob_overlap2x(('{"a":"temp.' || tbl_name || _subfix || '_pred","condition_a":"' || _condition || '","b":"coberturas.parcelas_tituladas","subfix":"_tit","tolerance":"0","add_diff":false, "temp": true}')::json);
        IF COALESCE( (tmp->>'features_inters_cnt')::int,0) > 0 THEN --> Si se han encontrado parcelas tituladas.
            sql := '
...
RETURN _out;
    ...
    EXCEPTION
		WHEN others THEN
			RAISE EXCEPTION 'geoSICOB (sicob_obtener_predio2x): _opt: % >> %, (%)', _opt::text, SQLERRM, sql;
