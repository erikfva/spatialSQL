CREATE OR REPLACE FUNCTION public.sicob_overlap2(_opt json)
 RETURNS json
 LANGUAGE plpgsql
AS $function$
DECLARE 
...
SELECT 
  ...
  'a.' || id__a || ' as id_a, CAST(''' || a::text || ''' as text) as source_a, 
  b.sicob_id as id_b, CAST(''' || b::text || ''' as text) as source_b,
...
DROP TABLE IF EXISTS a_intersect_b;
RAISE DEBUG 'Running %', sql;
EXECUTE 'CREATE TEMPORARY TABLE a_intersect_b ON COMMIT DROP AS ' || sql;

GET DIAGNOSTICS row_cnt = ROW_COUNT; -->obteniendo la cantidad de poligonos que se intersectan

IF row_cnt > 0 THEN --> Si existe interseccion
	--CREANDO EL INDICE ESPACIAL
	sql := 'CREATE INDEX a_intersect_b_the_geom ON a_intersect_b USING gist (the_geom public.gist_geometry_ops_2d)';	
END IF;
...
EXECUTE 'DROP TABLE IF EXISTS ' || tbl_subfixResult;
RAISE DEBUG 'Running %', sql;
IF COALESCE((_opt->>'temp')::boolean,FALSE) = TRUE THEN
...
EXECUTE 'ALTER TABLE ' || tbl_subfixResult || ' DROP COLUMN IF EXISTS sicob_id' ;
