CREATE OR REPLACE FUNCTION public.sicob_snap_edge_copy1(_edge geometry, _opt json)
 RETURNS geometry
 LANGUAGE plpgsql
 STRICT
AS $function$
...
_opt = ('{"target":"coberturas.predios_proceso_geosicob_geo_201607", "tolerance":"' || (6::float/111000)::text || '"}')::json;
	...
	_buffer2snap := st_union(ST_Buffer(_edge,  _tolerance * 2));
...
SELECT * FROM topo_face;

DROP TABLE IF EXISTS topo_face_to_snap;
CREATE TEMPORARY TABLE topo_face_to_snap ON COMMIT DROP AS
WITH
topo_face_ends AS (
	SELECT 
...
topo_face_ends_info AS (
	...
	FROM (
      SELECT *,
          ...
          as direction_line,
          sicob_shadowline(
              (SELECT the_geom FROM topo_face WHERE segment = t.segment), 
              ST_ShortestLine(
                  pt,_edge
              )
          ...
          CASE WHEN angle < 90 THEN
              CASE WHEN pos = 0 THEN
                  ST_LineInterpolatePoint(
                      (SELECT the_geom FROM topo_face WHERE segment = u.segment),
                      _tolerance/len 
                  )
              ELSE
                  ST_LineInterpolatePoint(
...
corner_cut_points_info AS (
        ...
        FROM (
          SELECT *, --Obteniendo el punto de interseccion entre cut_line y topo_face
              st_intersection(
                  cut_line,
				(SELECT the_geom FROM topo_face WHERE segment = ccl.segment)
              ) as cut_point
...
topo_face_cut_corner_fractions AS (
	SELECT 
          ...
          ccp.pos,
          st_linelocatepoint(
              (SELECT the_geom FROM topo_face WHERE segment = ccp.segment), 
              cut_point
          ) AS cp_fraction
	FROM
...
topo_face_cut_corner AS (
	SELECT segment,
    st_linesubstring(
    	(SELECT the_geom FROM topo_face WHERE segment = tfci.segment),
        startfraction,
        endfraction
    ) as the_geom
    FROM 
...
SELECT 
  i.id_segmenta,
  sicob_setpoint(
  sicob_setpoint(c.the_geom, i.posptoa,(SELECT t.pto FROM _ends_pts_segmentb t WHERE t.id_segment = i.id_segmentb AND t.pospto = i.posptob), (SELECT the_geom FROM topo_face_to_snap WHERE segment=i.id_segmentb limit 1)),
  i2.posptoa,
  (SELECT t.pto FROM _ends_pts_segmentb t WHERE t.id_segment = i2.id_segmentb AND t.pospto = i2.posptob),
  (SELECT the_geom FROM topo_face_to_snap WHERE segment=i.id_segmentb limit 1)
  )  AS the_geom
FROM
  _snapinf i
...
_snapclipa2 AS (  
  SELECT 
    c.segmenta,
    sicob_setpoint(c.the_geom, i.posptoa, (SELECT t.pto FROM _ends_pts_segmentb t WHERE t.id_segment = i.id_segmentb AND t.pospto = i.posptob), (SELECT the_geom FROM topo_face_to_snap WHERE segment=i.id_segmentb limit 1) ) AS the_geom
  FROM
    _clipa c
