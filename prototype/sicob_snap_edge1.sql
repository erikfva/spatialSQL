CREATE OR REPLACE FUNCTION public.sicob_snap_edge1(_edge geometry, _opt json)
 RETURNS geometry
 LANGUAGE plpgsql
 STRICT
AS $function$
...
---------------------------
-->	_edge: linea del borde que será ajustada hacia "target".
-->	_opt : cadena json con las sigtes. opciones :
-->		target : Capa de polígonos a la que será ajustado el borde "_edge"  
-->		tolerance : Distancia máxima(una aproximación a metros) para el ajuste.
-->		returnpolygon (true/false): Devuelve el resultado convertido a polígono. Si no se especifica devuelve una linea de borde.
-->		  
...
--CREATE TABLE temp._topo_face AS
WITH
line_pol_b AS (
...
--> REFLEJANDO LAS LINEAS DE topo_face A LOS BORDES DE _edge
